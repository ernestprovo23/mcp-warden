...
| **T-CAP-PAD** | An attacker who controls tool-result size **pads a malicious frame above `--max-frame-bytes`** so the over-cap fail-open path forwards it **uninspected** (§2.4) — a deliberate inspection bypass | The over-cap pass-through is the documented v0.3 contract (availability-over-inspection, §2.4 / §3.3 T-AVAIL): a server MUST NOT be able to break a session merely by emitting a huge frame, so `guard` forwards over-cap frames un-inspected rather than blocking. The cost is that the *same* mechanism lets an attacker who can inflate result size pad past the cap to skip inspection entirely. This is **fail-OPEN by deliberate choice**, not an oversight. | **Residual risk, accepted in v0.3.** No behavior change here: making the over-cap path fail-CLOSED under `--strict` (terminate or block the padded frame) is tracked in **#37**, now expanded to cover this padded-frame inspection-bypass explicitly. Operators who value integrity over availability for large results should follow #37. |
